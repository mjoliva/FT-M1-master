...
LinkedList.prototype.remove = function(){
  ...
  if(this.head.next === null){
    ...
    let capturarData = current.next.value;
    current.next = null;
    return capturarData;
...
LinkedList.prototype.search = function(arg){
  ...
  if(typeof arg != 'function'){
    functionCallback = function(valor){
      return valor === arg;
    }
  ...
  let current = this.head;
  while (current != null){
    if(functionCallback(current.value)){
...
(Luego de haber pasado todos los tests, a modo de ejercicio adicional, 
  pueden modificar un poco la clase para que 
...
La clase debe tener los siguientes métodos:
  ...
  - set: recibe el conjunto clave valor (como dos parámetros distintos),
   hashea la clave invocando al método hash, y 
...
Primero puedo chequear, con hasKey, 
si ya hay algo en la tabla con el nombre 'instructora';
 luego, invocando set('instructora', 'Ani'), 
 se almacenará el par clave-valor en un bucket específico 
...
HashTable.prototype.set = function(key, valor){
  ...
  var place = this.hash(key)

  this.contenedores[place] = this.contenedores[place] || [];
  this.contenedores[place].unshift({key: key, value: valor})
...
HashTable.prototype.get = function(key){
  ...
  for(let i = 0; i < this.contenedores[place].length; i++){
    if(this.contenedores[place][i].key === key){
      return this.contenedores[place][i].value;
    }
  }
  return false;
}
